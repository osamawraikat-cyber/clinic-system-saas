...
export async function sendInvitationEmail(email: string, inviteLink: string, clinicName: string) {
    ...
    try {
        await resend.emails.send({
            from: 'SehaTech <onboarding@resend.dev>', // Use resend.dev for testing if no domain
            to: email,
            subject: `Join ${clinicName} on SehaTech`,
            html: `
                <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
                    <h2>You've been invited!</h2>
                    <p>You have been invited to join <strong>${clinicName}</strong> on SehaTech.</p>
